...
void initRgbLed()
{
    ...
    // Start with LED off (HIGH for common anode)
...
// Only use pure red, green, or blue (no mixing)
static void setRgbColor(uint8_t r, uint8_t g, uint8_t b)
{
    // Common anode: LOW turns LED on, HIGH turns it off
    digitalWrite(RGB_PIN_RED, r > 0 ? LOW : HIGH);
    digitalWrite(RGB_PIN_GREEN, g > 0 ? LOW : HIGH);
    digitalWrite(RGB_PIN_BLUE, b > 0 ? LOW : HIGH);
}

...
bool initSdConfig()
{
    ...
    if (!LittleFS.begin(true))
    {
        ...
        logLine("Make sure to upload filesystem: pio run --target uploadfs");
        return false;
    }
    ...
    if (!configFile)
    {
        ...
        logLine("Upload config.json to filesystem: Place in data/ folder and run 'pio run --target uploadfs'");
        return false;
    }
    ...
    strncpy(g_deviceConfig.clientId, device["ClientId"] | "esp32-s3-001", MAX_DEVICE_ID_LEN - 1);
    strncpy(g_deviceConfig.deviceType, device["DeviceType"] | "ESP32-S3", MAX_DEVICE_TYPE_LEN - 1);
    strncpy(g_deviceConfig.friendlyName, device["FriendlyName"] | "ESP32-S3 Client", MAX_FRIENDLY_NAME_LEN - 1);
